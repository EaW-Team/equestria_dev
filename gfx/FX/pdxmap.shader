...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			float3 generalColorTest = tex2D(GradientBorderChannel1, float2(Input.uv.x, 1.0-(Input.uv.y * 0.5))).xyz - shieldActiveColor;
			if (abs(generalColorTest.x) < epsilon && abs(generalColorTest.y) < epsilon && abs(generalColorTest.z) < epsilon) {
